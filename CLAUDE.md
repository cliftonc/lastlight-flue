...
- **Transient model-error retry**: `runPhasePrompt` (`src/agent-lib/record-execution.ts`) — the ONE seam every phase prompt funnels through (build / explore / pr-review / issue-triage) — retries on **transient provider errors only** (OpenAI 5xx "An error occurred while processing your request", 429, 503, dropped sockets; `isTransientModelError`) with bounded exponential backoff, so a single blip can't kill a multi-minute run. Deterministic/client errors fail fast. Policy is overridable via `setPhaseRetryConfig` (tests inject a no-op `sleep`).
