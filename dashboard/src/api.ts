...
/**
 * One derived phase of a workflow run (GET /workflow-runs/:id/phases). Flue
 * workflows carry no declarative phases, so the pipeline is reconstructed from
 * the run's event stream: each phase that runs an agent is one operation
 * (`operationId`), labeled by its harness session name. Single-agent workflows
 * appear as a single `default` phase.
 */
export interface RunPhase {
  /** Stable operation key — used to filter this phase's slice of the run log. */
  operationId: string;
  /** Phase name (harness session name; `default` for single-agent runs). */
  name: string;
...
export const api = {
  ...
  /** Phases derived from a run's event stream (one operation per agent phase). */
  runPhases: (id: string) =>
    req<{ phases: RunPhase[] }>(`/workflow-runs/${encodeURIComponent(id)}/phases`),
