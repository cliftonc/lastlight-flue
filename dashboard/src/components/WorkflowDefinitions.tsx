...
type ViewTab = "diagram" | "source";
const VIEW_TABS = ["diagram", "source"] as const;

/**
 * Workflow browser. Lists every workflow module under `src/workflows/` and shows
 * the selected one as either a React Flow diagram (its pipeline, derived from the
 * most recent run — Flue workflows carry no declarative phases) or its raw
 * TypeScript source. Clicking a phase node reveals what that step recorded, with
 * any linked skill/prompt rendered inline below.
 */
export function WorkflowDefinitions() {
  ...
  // Phases derived from the workflow's most recent run (Flue has no declarative
  // phases). `null` = still loading; `[]` = no runs yet to derive from.
  const [derivedPhases, setDerivedPhases] = useState<WorkflowFullPhase[] | null>(null);
  const [derivedFromRun, setDerivedFromRun] = useState<string | null>(null);
  ...
  useEffect(() => {
    if (!selectedName) {
      ...
      setDerivedPhases(null);
      setDerivedFromRun(null);
      return;
    }
    let cancelled = false;
    setDefinition(null);
    ...
    // Derive the pipeline from the most recent run of this workflow. Flue
    // workflows have no declarative phases, so the diagram reflects what the
    // latest run actually executed (one node per agent operation).
    api
      .workflowRuns({ workflow: selectedName, limit: 1 })
      .then(async (res) => {
        const latest = res.workflowRuns[0];
        if (!latest) {
          if (!cancelled) setDerivedPhases([]);
          return;
        }
        const { phases } = await api.runPhases(latest.id);
        if (cancelled) return;
        setDerivedFromRun(latest.id);
        setDerivedPhases(
          phases.map(
            (p: RunPhase): WorkflowFullPhase => ({
              name: p.operationId,
              label: p.name,
              type: "agent",
            }),
  ...
  // The definition fed to the diagram has its (empty) phases replaced by the
  // run-derived pipeline.
  const diagramDefinition = useMemo<WorkflowFullDefinition | null>(() => {
    if (!definition) return null;
    return { ...definition, phases: derivedPhases ?? [] };
  }, [definition, derivedPhases]);

  const selectedPhase = useMemo<WorkflowFullPhase | null>(() => {
    if (!diagramDefinition || !selectedPhaseName) return null;
    return diagramDefinition.phases.find((p) => p.name === selectedPhaseName) ?? null;
  }, [diagramDefinition, selectedPhaseName]);
      ...
      {selectedName ? (
        <div className="flex-1 overflow-hidden flex flex-col p-4 gap-3 min-h-0">
          ...
          <div className="flex-1 overflow-hidden min-h-0">
            ...
            {view === "diagram" && !definitionError && derivedPhases === null && (
              <div className="p-4 text-sm text-base-content/40">deriving pipeline…</div>
            )}
            {view === "diagram" && !definitionError && derivedPhases?.length === 0 && (
              <div className="flex-1 flex items-center justify-center text-base-content/40 text-sm text-center p-6">
                No runs yet — run this workflow to see its pipeline. Flue workflows have
                no static phase definition; the diagram is derived from a run.
              </div>
            )}
            {view === "diagram" && diagramDefinition && derivedPhases && derivedPhases.length > 0 && (
              <div className="flex flex-col h-full min-h-0">
                {derivedFromRun && (
                  <div className="text-2xs text-base-content/40 font-mono mb-1 shrink-0">
                    pipeline derived from run {derivedFromRun}
                  </div>
                )}
            ...
            {view === "source" && yamlError && (
              <div className="p-4 text-sm text-error border border-error/40 bg-error/5 rounded">
                {yamlError}
              </div>
            )}
            {view === "source" && yamlText !== null && (
              <div className="h-full overflow-auto">
                <CodeBlock code={yamlText} language="typescript" />
              </div>
            )}
          </div>
        </div>
