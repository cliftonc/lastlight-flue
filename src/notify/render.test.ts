...
import { renderProgress, collapseDetail, STATUS_EMOJI } from "./render.ts";
import type { ProgressModel } from "./types.ts";
...
describe("collapseDetail", () => {
  it("keeps a short-label markdown link intact even when the URL is long (regression)", () => {
    // The exact shape from a build plan: short link text, very long branch-encoded URL.
    const url =
