/**
 * Convert standard Markdown to Slack's mrkdwn format. The shared renderer
 * (`render.ts`) emits GitHub-flavored markdown; the Slack transport runs it
 * through here before `chat.update` / `chat.postMessage` so headings, bold,
 * links and tables render legibly in Slack (which has no header/table mrkdwn).
 *
 * Ported verbatim from ~/work/lastlight/src/connectors/slack/mrkdwn.ts.
 */
...
function convertMarkdownTables(text: string, render: (block: string[]) => string): string {
  ...
  while (i < lines.length) {
    ...
    if (next !== undefined && isTableRow(line) && TABLE_DELIMITER.test(next)) {
      const block = [line, next];
      let j = i + 2;
      for (let row = lines[j]; row !== undefined && isTableRow(row); row = lines[++j]) {
        block.push(row);
      }
