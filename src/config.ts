...
import { normalizeAllowlistHost } from "./engine/egress-allowlist.ts";
...
/**
 * Flue reasoning effort levels (the agent `thinkingLevel` option). These are
 * exactly Last Light's variant vocabulary, so `resolveVariant` maps 1:1 onto
 * `thinkingLevel`. Verified against the installed @flue/runtime 1.0.0-beta.2 /
 * docs/guide/models.md: 'off' | 'minimal' | 'low' | 'medium'(default) |
 * 'high' | 'xhigh'.
 */
...
const BASE_THINKING: ThinkingLevel = "medium";
...
function applyJsonStringMap(
  ...
  try {
    ...
    const msg = err instanceof Error ? err.message : String(err);
    console.warn(`[config] Invalid ${label} JSON: ${msg}`);
  }
}
...
/**
 * Resolve the model specifier (`provider/model`) for a task type, falling back
 * to the configured default. Two forms:
 *   resolveModel(models, task) — pure, used in tests and where a config is in hand.
 *   resolveModel(task)         — convenience, reads the loaded runtime config.
 */
export function resolveModel(models: ModelConfig, taskType: string): string;
export function resolveModel(taskType: string): string;
export function resolveModel(a: ModelConfig | string, b?: string): string {
  if (typeof a === "string") {
    const cfg = currentConfig ?? loadConfig();
    return resolveModelFrom(cfg.models, a);
  }
  return resolveModelFrom(a, b ?? "");
}

function resolveModelFrom(models: ModelConfig, taskType: string): string {
  return models[taskType] || models.default;
}

/**
 * Resolve the raw variant string for a task type (may be undefined). Kept for
 * back-compat with the reference's vocabulary; `resolveThinking` is the typed,
 * Flue-facing form that guarantees a valid `thinkingLevel`.
 */
...
/**
 * Resolve the Flue `thinkingLevel` (reasoning effort) for a task type. Maps the
 * configured variant 1:1 onto Flue's level vocabulary
 * ('off'|'minimal'|'low'|'medium'|'high'|'xhigh'); an unset or unrecognized
 * variant fails open to BASE_THINKING ('medium', Flue's default). Two forms:
 *   resolveThinking(variants, task) — pure, used in tests / with a config in hand.
 *   resolveThinking(task)           — convenience, reads the loaded runtime config.
 */
export function resolveThinking(variants: VariantConfig, taskType: string): ThinkingLevel;
export function resolveThinking(taskType: string): ThinkingLevel;
export function resolveThinking(a: VariantConfig | string, b?: string): ThinkingLevel {
  if (typeof a === "string") {
    const cfg = currentConfig ?? loadConfig();
    return resolveThinkingFrom(cfg.variants, a);
  }
  return resolveThinkingFrom(a, b ?? "");
}

function resolveThinkingFrom(variants: VariantConfig, taskType: string): ThinkingLevel {
  const raw = resolveVariant(variants, taskType);
  return isThinkingLevel(raw) ? raw : BASE_THINKING;
}
