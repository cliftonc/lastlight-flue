import type { GitHubTokenPermissions } from "./git-auth.ts";
import type { OtelConfig, SandboxBackend } from "../config.ts";
...
// TODO(persona): the reference `loadAgentContext()` delegated to
// `workflows/loader.ts` (unported) and is superseded by
// `src/agents/persona.ts` (`loadPersona()`), which lands in a later Phase 1
// slice — see the Phase 1 port map in PROGRESS.md / design/phase-1-shared-core.md.
// Intentionally NOT ported here: nothing in the Flue agents should call the old
// signature (design Q1.4). Re-export persona's `loadPersona()` from there once it exists.
