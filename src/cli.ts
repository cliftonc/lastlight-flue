 ...
 *
 * Ported near-verbatim from `~/work/lastlight/src/cli.ts` (Phase 2 CLI port).
 * Adaptations: `.js`→`.ts` import specifiers; `noUncheckedIndexedAccess` guards
 * (non-null assertions / locals where the index is provably in range); the
 * `setup` wizard (`./setup.ts`, 844L interactive first-run flow) is NOT ported
 * this slice — `lastlight setup` reports that gracefully (see TODO below). The
 * CLI is framework-independent (fetch + chalk); several endpoints it targets
 * still return 501 in this build (stats/sessions/log-search/approvals/
 * executions) — those commands surface the server's error at runtime, as
 * intended; they are NOT faked or removed.
 */
...
import { table, age, colorStatus, checkmark, followSSE } from "./cli-format.ts";
import { renderTimeline, renderMessage, renderRaw } from "./cli-timeline.ts";
...
async function cmdSkill(name: string): Promise<void> {
  ...
  if (!targetRef) die(`Usage: lastlight ${name} <owner/repo>${repoLevelOnly ? "" : " | <owner/repo#N>"}`);
  const parsed = repoLevelOnly ? null : parseGitHubRef(targetRef);
  ...
  if (parsed) {
    ...
    context = { repos: [targetRef], mode: "scan" };
    if (!JSON_OUT) console.log(`Triggering ${name} scan on ${targetRef}…`);
  }
...
async function main() {
  ...
  switch (cmd) {
    case "setup": {
      // TODO(phase-2/cli-setup): the interactive first-run wizard
      // (`~/work/lastlight/src/setup.ts`, 844L) is NOT ported in this slice —
      // it wires local docker/secrets state and is out of scope for the thin
      // HTTP-client port. Until it lands, point operators at `login`.
      die("`lastlight setup` is not available yet in this build. Use `lastlight login` to authenticate against a running instance.");
      return;
    }
